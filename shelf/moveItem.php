...
<head>
    ...
    <link rel="stylesheet" href="../css/style-changeShelf.css">
...
<body>
        ...
        <div class="details">
          <div class="QrForm">
            <div class="cardHeader">
              <h2>QR Change Shelf</h2>    
            </div>
            <div class="qrcontainer">
                <button type="submit" class="btn" onclick="openPopup()"><span class="icon"><span class="eicon-qrcode"></span></ion-icon></span></button>
            </div> 
            
            
            <form method="POST" action="changeShelf.php">
              <div class="row">
                  <div class="col-md-2 mb-3 mt-3">
                    <input type="text" name="prodID" id="prodID" class="scanner-data" placeholder="Product ID" required>
                  </div>
                  <div class="col-md-2 mb-3 mt-3">
                    <input type="text" name="prodName" id="prodName" class="scanner-data" placeholder="Product name" required>
                  </div>
              </div>  
              
              <button type="submit" id="next" class="nextBtn">Next</button>
              
            </form>
            <button type="submit" class="clearbtnOut" onclick="clearbtn()">Clear</button>
    ...
    <script>
    function clearbtn(){
        window.location.href='moveItem.php'
    }
    </script>
        ...
        function openPopup() {
            ...
            scanner.addListener('scan', function(content) {
                if (currentFieldIndex < scannerDataFields.length) {
                    if (content.includes(',')) {
                        let reqno = content.split(',')[0];
                        let partcode = content.split(',')[1];
                        scannerDataFields[0].value = reqno;
                        scannerDataFields[1].value = partcode;
                    }else{
